...
TypeScript escribe mas codigo regulamente que javascript ahciendolo mas pesado pero con uan buena razon y es la seguridad de tipos*/
...
type HeroId = `${string}-${string}-${string}-${string}-${string}`; //Tipo que representa un UUID, esto es un templaete union types
type HeroPowerScale = "local" | "planetario" | "galactico" | "intergalactico"; //ESto son los possibles que valores que podra tener este tipo (Union types)

type Hero = {
  readonly id?: HeroId; //Con el readonly se indica que esa propiedad no se puede modificar, y con el ? que es opcional, no es inmutable puesto que lo uncioq eu hace es que typescript nos amrque un error que no se uede hacer pero en ralidad cuando creamos el javascript si lo permite porque no existe en tiemo de ejecucion, par aeso se usaria algo de js como object.freeze
  name: string;
  age: number;
  powers: string[];
  isActive?: boolean;
  powerScale?: HeroPowerScale; //Con esto limitamos los posibles valores que puede tener esa propiedad
};
...
const color: HexadecimalColor = "#ff5733";
//const color2: HexadecimalColor = "ff5733AA"; //Da error porque no comienza con #

//Intersection types

type HeroWithPowerScale = Hero & { powerScale: HeroPowerScale }; //Es un nuevo tipo combinacion de otros.

//Type from value
const address: { [key: string]: string | number } = {
  street: "123 Main St",
  city: "Anytown",
  zip: 12345,
};

type Address = typeof address; // Typeof aqui obtieen la estuctura del tipo que se dice para podelo reutilizar
...
function crearDireccion(street: string, city: string, zip: number) {
  return {
    street,
    city,
    zip,
...
type AddressFromFunction = ReturnType<typeof crearDireccion>; //Obtiene el tipo de retorno de la función

const address3: AddressFromFunction = {
  street: "789 Oak St",
  city: "Sometown",
  zip: 54321,
};
...
type CellValue = "x" | "o" | "";
type GameBoard = [
  //ESto es una tupla, en TypeScript es un array con una longitud fija y elementos posicionales.
...
type RGB = [number, number, number]; //Tupla que dice que son 3 posiciones y numeros
const colorPrimary: RGB = [255, 87, 51];
